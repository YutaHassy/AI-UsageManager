 ...
 *
 * **いま残っているのは remove だけで、使うのも extension.js の1箇所だけ
 * です。** 元は「同じ文言を2つのファイルが書いていて、綴りが一字ずれたら
 * 片方だけ訳が当たらなくなる」のを防ぐための置き場でした。追加・編集・
 * 再ログインが画面の中のフォームへ移り、進み具合を名指しで知らせる相手が
 * いなくなったので (別ウィンドウを待つ仕組みごと畳みました)、その事故の
 * 起きようがない形になっています。**2箇所目ができたら、また意味を持ちます。**
 */
